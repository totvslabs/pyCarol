...
class MyApp(CarolApp):
    ...
    def run_me(self):
        animals = {'Dog': 92, 'Cat': 45}

        l0 = timer()
        self.carol.storage.save("animals-sync2", animals)
        l1 = timer()
        print("Sync: {}s".format(l1 - l0))

        l0 = timer()
        self.carol.storage.save_async("animals-async2", animals)
        l1 = timer()
        print("Async: {}s".format(l1 - l0))
